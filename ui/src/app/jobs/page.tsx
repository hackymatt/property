...
export default function JobsPage() {
  ...
  const datasource = useMemo<IDatasource>(() => ({
    getRows(params: IGetRowsParams) {
      ...
      queryClient.fetchQuery({
        queryKey: ["jobs", qp],
        queryFn: () => jobs.list(qp).then((r) => r.data),
        staleTime: 30_000,
      })
        .then((data) => { params.successCallback(data.results, data.count); setErrorRef.current(""); })
        .catch(() => { params.failCallback(); setErrorRef.current("Failed to load jobs"); });
    },
  }), [queryClient]);
  ...
  const handleDelete = async () => {
    ...
    await jobs.remove(deleteTarget.id);
  ...
  const colDefs: ColDef<Job>[] = [
    ...
    {
      field: "stage", width: 130, filter: true,
  ...
  return (
    <AppLayout>
      ...
      {contextMenu}
      <JobDialog open={dialogOpen} job={selected} onClose={() => { setDialogOpen(false); setSelected(null); }} onSave={handleSave} />
      <ConfirmDialog
        open={!!deleteTarget}
        title="Delete Job"
        message={`Delete job "${deleteTarget?.source}"?`}
